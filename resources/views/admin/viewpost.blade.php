        ...
        <div class="row mt">
          <div class="col-lg-12 bg-light" style="margin-top:0px; width:100%">
            <form method="get" action="{{ route('edit-post', ['post' => $post->slug]) }}" enctype="multipart/form-data" files="true">
                ...
                <div class="form-group">
                    <label class="col-md-5">Author: <span class="bold">{{ $post->user->name }}</span></label>
                    <label class="col-md-5 text-right">Category: <span class="strong">{{ $post->category->title }}</span></label>
                </div>
                <div class="form-group mt-2">
                    <label class="col-md-12"><img src="{{ asset($post->image) }}" alt = "Post image" width="100%" height="700px" /></label>
                </div>
